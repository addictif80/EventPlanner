...
class Invoice extends Model
{
    ...
    {
        $stmt = Database::connection()->prepare('SELECT * FROM invoice_items WHERE invoice_id = ? ORDER BY position ASC, id ASC');
    ...
    {
        $stmt = Database::connection()->prepare('SELECT * FROM payments WHERE invoice_id = ? ORDER BY payment_date DESC, id DESC');
        $stmt->execute([$invoiceId]);
        return $stmt->fetchAll();
    }
    ...
    public static function replaceItems(int $invoiceId, array $items): void
    {
        ...
        $stmt = $pdo->prepare(
            'INSERT INTO invoice_items (invoice_id, description, quantity, unit_price, total, position) VALUES (?, ?, ?, ?, ?, ?)'
        );
        foreach ($items as $i => $item) {
            $stmt->execute([$invoiceId, $item['description'], $item['quantity'], $item['unit_price'], $item['total'], $i]);
        }
    }
    ...
    public static function recalculatePaidStatus(int $invoiceId): void
    {
        ...
        $update = $pdo->prepare('UPDATE invoices SET amount_paid = ?, status = ? WHERE id = ?');
        $update->execute([$paid, $status, $invoiceId]);
    }
}
